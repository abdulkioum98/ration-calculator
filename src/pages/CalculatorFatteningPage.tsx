...
import { FatteningCowData } from './FatteningInfoPage';
...
interface CalculatorFatteningPageProps {
  fatteningData?: FatteningCowData | null;
  onEditInfo?: () => void;
}

export default function CalculatorFatteningPage({ fatteningData, onEditInfo }: CalculatorFatteningPageProps) {
  ...
  // DM Ratio Reference Logic for Fattening
  let stageName = 'Growing Animal (6-24M)';
  ...
  // Fattening Required Formulas
  const dmReq = bodyWeight * 0.03;
  const cpReq = ((bodyWeight * 1) + (targetGainPerDay * 0.5)) / 1000;
  const energyReq = (bodyWeight * 0.10) + (targetGainPerDay * 0.10);
      ...
      <div className="bg-emerald-50/60 rounded-xl p-3.5 sm:p-4 shadow-xs border border-emerald-200/80 flex justify-between items-center gap-3">
        <div className="space-y-1.5 min-w-0">
          ...
          <div className="flex items-center gap-x-3 gap-y-1 text-slate-700 text-[11px] sm:text-xs flex-wrap font-semibold">
            ...
            <span className="text-slate-300">•</span>
            <span><b>Target Gain:</b> {targetGainPerDay || 0} g/day</span>
